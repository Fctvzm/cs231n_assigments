...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (gamma, xhat, ivar, xmean, sqrtvar, var, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    N, D = dout.shape
    gamma, xhat, ivar, xmean, sqrtvar, var, eps = cache
    ###########################################################################
    ...
    dxhat = gamma * dout #(D, ) * (N, D) = (N, D)
    dxmean = dxhat * ivar #(N, D) * (D, ) = (N, D)
    divar = np.sum(dxhat * xmean, axis = 0) #(D, )
    dsqrtvar = -divar / (sqrtvar ** 2)
    dvar = 0.5 * (dsqrtvar / np.sqrt(var + eps))
    dsqrt = (1 / N) * np.ones((N, D)) * dvar
    dxmean2 = 2 * xmean * dsqrt
    dx1 = dxmean + dxmean2
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(0, height):
        ...
        for j in range(0, width):
            jx = stride * j
            out[:, :, i, j] = np.dot(x[:, :, ix:(HH + ix), jx:(WW + jx)].reshape(N, -1), w.reshape(F, -1).T) + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(0, H):
        ...
        for j in range(0, W):
            jx = stride * j
            dout_slice = dout[:, :, i, j]
            dx[:, :, ix:(HH + ix), jx:(WW + jx)] += np.dot(dout_slice, w.reshape(F, -1)).reshape(N, C, HH, WW)
            dw += np.dot(dout_slice.T, x[:, :, ix:(HH + ix), jx:(WW + jx)].reshape(N, -1)).reshape(w.shape)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(HH):
        ...
        for j in range(WW):
            jx = stride * j
            out[:, :, i, j] = np.amax(x[:, :, ix:(HH + ix), jx:(WW + jx)], axis=(2,3))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
